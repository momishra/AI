...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None: #checking if the ghost is in board or not 
            temp=self.getJailPosition()
            allPossible[temp]=1.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions :
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newpos in newPosDist.items():
                p=newpos[0]
                Pr=newpos[1]
                allpossible[newpos[0]] = allpossible[newpos[0]]+self.beliefs[oldPos]*newpos[1]
